...
Now that we were able too receive data from Terra, we had to connect wearable account to the webapp. Using terra's widget we can connect too different wearables and complete the authentification process.
...
* 3 - Implemented the database flow to store and retrieve data & started adding unique ids for each session (each sessions we generate an id through a hash and store it with wearable id show code) 
...
* 6 - Analytics pages for further analysis and vsualisation 
